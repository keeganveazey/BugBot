...
if gpus:
    try:
        for gpu in gpus:
            tf.config.experimental.set_memory_growth(gpu, True)  # Prevents full GPU memory allocation
        tf.config.experimental.set_visible_devices(gpus[0], 'GPU')  # Use only the first GPU
        print("GPU is enabled and set for TensorFlow.")
    except RuntimeError as e:
        print(e)
else:
    print("No GPU found. Ensure CUDA and TensorFlow-GPU are installed.")
...
if gpus:
    try:
        for gpu in gpus:
            tf.config.experimental.set_memory_growth(gpu, True)  # Prevents TensorFlow from allocating all GPU memory
...
if __name__ == "__main__":
    ...
    # run 4) python DenseNet201_run_tuner_script_updated.py --epochs 20 --patience 3 --min_delta 0.001 --executions_per_trial 1 --max_trials 20
